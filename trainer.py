...
import config
import data_manager
from measure_process import measure_process_prediction
...
def main():
    ...
    for universe_name, tickers in config.UNIVERSES.items():
        print(f"\n=== Universe: {universe_name} (Measure-Valued Process) ===")
        ...
        for win in config.WINDOWS:
            ...
            for etf in tickers:
                if etf not in returns.columns:
                    continue
                pred = measure_process_prediction(returns[etf], win,
    ...
    local_path = Path(f"results/measure_process_{today}.json")
    with open(local_path, "w") as f:
        json.dump({"run_date": today, "universes": all_results}, f, indent=2)

    import push_results
    push_results.push_daily_result(local_path)
    print("\n=== Measure-Valued Process Engine complete ===")
